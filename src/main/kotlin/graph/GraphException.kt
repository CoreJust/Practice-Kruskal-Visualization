...
class EdgeAlreadyExistsException(fromVertexName: String, toVertexName: String) :
    GraphException("Such an edge already exists: ($fromVertexName - $toVertexName)")

class SelfLoopException(vertexName: String) :
    GraphException("No self-loops are allowed in the graph, tried to add a self-loop to vertex $vertexName")

class NoSuchVertexException(vertexName: String) : GraphException("No such vertex exists in graoh: $vertexName")
class NoSuchEdgeException(fromVertexName: String, toVertexName: String) :
    GraphException("No such edge exists in graoh: ($fromVertexName - $toVertexName)")
